...
exports.subscribe= function(channel, /*String?*/eventName, listener){
	...
	if(newChannel){
		var responses = exports.publish(channel, {channel:channel, event: "monitored", monitored: true, forEvent: eventName});
	}else{
		var responses = [];
	}
	forOnlyOneOtherClient(subscribers, {channel:channel, event: "monitored", monitored: true, forEvent: eventName});
		...
		if(eventName && eventName != "*"){
			...
			if(subSubscribers){
				...
				if(subSubscribers.length === 0){
					...
					exports.publish(channel, {event:"monitored", forEvent: eventName, monitored: false});
				}else{
					forOnlyOneOtherClient(subSubscribers, {channel:channel, event: "monitored", monitored: false, forEvent: eventName});
				}
			...
			if(subscribers.length === 0){
				...
				forOnlyOneOtherClient(subscribers, {channel:channel, event: "monitored", monitored: false});
			}
		}
	}
};
